...
func _handle_interaction():
	if currentLabel == 0:
		open = false
		var node = preload("res://Scenes/Party.xml").instance()
		get_node("/root/World").add_child(node)
	if currentLabel == 1:
		open = false
		#var node = preload("res://Scenes/Inventory.xml").instance()
		#get_node("/root/World").add_child(node)
...
func pointer_update():
	pointer.set_global_pos(Vector2(pointer.get_global_pos().x,labels[currentLabel].get_global_pos().y+18))
